...
@TeleOp(name = "RoverVuforia_V01_50 (Blocks to Java)", group = "")
public class RoverVuforia_V01_50 extends LinearOpMode {
  ...
  private void Calc_Display() {
    ...
    if (RedWall()) {
      // Fix zRotation if facing red Wall
      if (avgRotZ >= -10 && avgRotZ <= 10) {
        if (avgRotZ < 0) {
          // zRotation= -7 deg to -1 deg change to -173 deg to -179 deg
          avgRotZ = -(180 + avgRotZ);
        } else {
          // zRotation= -0 deg to +7  deg change to 180 deg to 173 deg
          avgRotZ = 180 - avgRotZ;
        }
      }
      FormatXYZ_string();
  ...
  private void Calc_vector_to_Wall_Target(double TourTargX, double TourTargY) {
    ...
    zTourLen = Math.sqrt(Math.pow(zTourDeltaX, 2) + Math.pow(zTourDeltaY, 2));
    Format_Tour_string();
  }
  ...
  private void Format_Tour_string() {
    // TODO: Enter the type for variable named zTourString
    UNKNOWN_TYPE zTourString;

    zTourString += "Tour DeltaX: " + JavaUtil.formatNumber(zTourDeltaX, 0);
    zTourString += " DeltaY]: " + JavaUtil.formatNumber(zTourDeltaY, 0);
    zTourString += " DeltaLen: " + JavaUtil.formatNumber(zTourLen, 0);
    zTourString += " Delta RotanionQ:  " + JavaUtil.formatNumber(zTourRotation, 0);
  }
